namespace EasyAbp.EzGet.NuGet.ServiceIndexs
{
    public static class ServiceIndexUrlConsts
    {
        public const string ServiceIndexUrl = "v3/index.json";
        public const string PackagePublishUrl = "api/v2/package";
        public const string SearchQueryServiceUrl = "v3/search";
        public const string RegistrationsBaseUrlUrl = "v3/registration";
        public const string PackageBaseAddressUrl = "v3/package";
        public const string AutoCompleteUrl = "v3/auto/complete";
    }
}
